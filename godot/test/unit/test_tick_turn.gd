extends GutTest


const CARD_A_PATH = "res://resources/cards/0001.tres"
const CARD_B_PATH = "res://resources/cards/0002.tres"

func _create_test_unit(card_path: String, owner: UserRes) -> UnitGame:
	var card = load(card_path) as CardRes
	assert_not_null(card, "No se pudo cargar: " + card_path)
	return UnitGame.new(card, owner)
func _assert_subscribed(tm: TurnManager, unit: UnitGame, expected: bool, msg: String) -> void:
	var connected = tm.tick_turn.is_connected(unit.advance_turn)
	assert_eq(connected, expected, msg)

func _set_unit_used(unit: UnitGame, hab: HabilityRes, cooldown: int) -> void:
	unit._habilities[hab] = cooldown
	unit.has_moved_this_turn = true
	unit.has_hability_this_turn = true

func _create_poison() -> AlterStateRes:
	var state = AlterStateRes.new()
	state.stat = StatData.new()
	state.stat.name = &"hp"
	state.stat.isPercent = false
	state.value = -5.0
	state.hitP = 1.0
	state.duration = 3
	state.objectiu = HabilityRes.SEL_SELF_FLAG
	return state
	
func _advance_and_wait(tm: TurnManager) -> void:
	tm.advance_turn()
	await wait_physics_frames(2)

func _setup_game_environment() -> Dictionary:
	var gr := GameResources.load_from()
	var map := TestMapGame.new().create_test_map()
	GameManager._gameMap = map
	GameManager._user_a = gr.users[0]
	GameManager._user_b = gr.users[1]
	GameManager._army_a = gr.users[0].obtener_ejercito_activo()
	GameManager._army_b = gr.users[1].obtener_ejercito_activo()
	
	var prev_add_target = gut.add_children_to
	gut.add_children_to = get_tree().get_root()
	var instance := preload("res://scenes/ingame/game_scene.tscn").instantiate() as TurnManager
	instance.turn_order = [gr.users[0], gr.users[1]]
	instance.turn_number = 0
	add_child_autoqfree(instance)
	
	return {
		"tm": instance,
		"map": map,
		"user_a": gr.users[0],
		"user_b": gr.users[1],
		"prev_add_target": prev_add_target,
	}
#func test_advance_turn_updates_units() -> void:
	#var env := _setup_game_environment()
	#
	#var unit_a = _create_test_unit(CARD_A_PATH, env.user_a)
	#var unit_b = _create_test_unit(CARD_B_PATH, env.user_b)
	#
	#env.map.place_unit(unit_a, Vector2i(5, 5))
	#env.map.place_unit(unit_b, Vector2i(10, 5))
		## ── Test 1: Suscripción a tick_turn ──
	#_assert_subscribed(env.tm, unit_a, true, "Unit A debe estar suscrita")
	#_assert_subscribed(env.tm, unit_b, true, "Unit B debe estar suscrita")
	#
	## ── Test 2: Cooldowns y flags se resetean ──
	#var hab_a = unit_a._cardRes.habilities[0]
	#_set_unit_used(unit_a, hab_a, 2)
	#
	#assert_eq(unit_a._habilities[hab_a], 2, "Cooldown inicial debe ser 2")
	#assert_true(unit_a.has_moved_this_turn, "Debe haber movido")
	#assert_true(unit_a.has_hability_this_turn, "Debe haber usado habilidad")
	#
	## ── Test 3: Turno de user_a — Unit A avanza, Unit B no ──
	#var hab_b = unit_b._cardRes.habilities[0]
	#_set_unit_used(unit_b, hab_b, 3)
	#
	#_advance_and_wait(env.tm)
	#
	#assert_eq(unit_a._habilities[hab_a], 1, "Cooldown A debe bajar a 1")
	#assert_false(unit_a.has_moved_this_turn, "A debe poder moverse")
	#assert_false(unit_a.has_hability_this_turn, "A debe poder usar habilidad")
	#assert_eq(unit_b._habilities[hab_b], 3, "Cooldown B no debe cambiar")
	#assert_true(unit_b.has_moved_this_turn, "B no debe resetearse aún")
	#
	## ── Test 4: Turno de user_b — Unit B avanza ──
	#_advance_and_wait(env.tm)
	#
	#assert_eq(unit_b._habilities[hab_b], 2, "Cooldown B debe bajar a 2")
	#assert_false(unit_b.has_moved_this_turn, "B debe poder moverse")
	#
	## ── Test 5: Pasivas se ejecutan al avanzar turno ──
	#var hp_before_a = unit_a.hp
	#unit_a.hp -= 30  # dañar para que la regeneración tenga efecto
	#var hp_damaged = unit_a.hp
	#
	#_advance_and_wait(env.tm)  # turno user_a — pasiva regen de unit_a se activa
	#
	## La pasiva de regeneración debería haber curado
	#assert_gt(unit_a.hp, hp_damaged, "Pasiva regen debe curar a Unit A")
	#
	## ── Test 6: Alter states expiran ──
	#var poison = _create_poison()
	#unit_a._currentAlterStates[poison] = 1
	#
	#_advance_and_wait(env.tm)  # turno user_b (unit_a no avanza)
	#_advance_and_wait(env.tm)  # turno user_a (unit_a avanza, veneno expira)
	#
	#assert_false(unit_a._currentAlterStates.has(poison), "Veneno debe haber expirado")
	#
	## ── Test 7: Alter states aplican efecto cada turno ──
	#var poison2 = _create_poison()
	#unit_b._currentAlterStates[poison2] = 3
	#var hp_before_poison = unit_b.hp
	#
	#_advance_and_wait(env.tm)  # turno user_b — veneno aplica daño
	#
	#assert_lt(unit_b.hp, hp_before_poison, "Veneno debe dañar a Unit B")
	#assert_true(unit_b._currentAlterStates.has(poison2), "Veneno aún tiene turnos")
	#assert_eq(unit_b._currentAlterStates[poison2], 2, "Veneno debe tener 2 turnos restantes")
	#
	## ── Test 8: Kill desuscribe ──
	#unit_b.kill()
	#await wait_physics_frames(2)
	#
	#_assert_subscribed(env.tm, unit_b, false, "Unit B muerta no debe estar suscrita")
	#assert_null(unit_b._tile, "Unit B muerta no debe tener tile")
	#
	## ── Test 9: Cooldown llega a 0 — habilidad disponible ──
	#unit_a._habilities[hab_a] = 1
	#_advance_and_wait(env.tm)  # turno user_b
	#_advance_and_wait(env.tm)  # turno user_a — cooldown baja a 0
	#
	#assert_eq(unit_a._habilities[hab_a], 0, "Cooldown debe llegar a 0")
	#assert_true(hab_a in unit_a.get_available_habilities(), "Habilidad debe estar disponible")
	#
	#gut.pause_before_teardown()
	#pass_test("advance_turn, passives, and tick_turn work correctly")
	##env.cleanup.call()

func test_advance_turn_updates_units() -> void:
	var env := _setup_game_environment()
	var tm: TurnManager = env.tm
	var map: MapGame = env.map
	
	var unit_a = _create_test_unit(CARD_A_PATH, env.user_a)
	var unit_b = _create_test_unit(CARD_B_PATH, env.user_b)
	
	map.place_unit(unit_a, Vector2i(5, 5))
	map.place_unit(unit_b, Vector2i(10, 5))
	
	await wait_until(func(): return tm.is_inside_tree(), 5)
	await wait_physics_frames(2)
	
	# ── Test 1: Suscripción a tick_turn ──
	_assert_subscribed(tm, unit_a, true, "Unit A debe estar suscrita")
	_assert_subscribed(tm, unit_b, true, "Unit B debe estar suscrita")
	
	# ── Test 2: Cooldowns y flags se resetean ──
	var hab_a = unit_a._cardRes.habilities[0]
	_set_unit_used(unit_a, hab_a, 2)
	
	# ── Test 3: Turno de user_a — Unit A avanza, Unit B no ──
	var hab_b = unit_b._cardRes.habilities[0]
	_set_unit_used(unit_b, hab_b, 3)
	
	await _advance_and_wait(tm)
	
	assert_eq(unit_a._habilities[hab_a], 1, "Cooldown A debe bajar a 1")
	assert_false(unit_a.has_moved_this_turn, "A debe poder moverse")
	assert_false(unit_a.has_hability_this_turn, "A debe poder usar habilidad")
	assert_eq(unit_b._habilities[hab_b], 3, "Cooldown B no debe cambiar")
	assert_true(unit_b.has_moved_this_turn, "B no debe resetearse aún")
	
	# ── Test 4: Turno de user_b — Unit B avanza ──
	await _advance_and_wait(tm)
	
	assert_eq(unit_b._habilities[hab_b], 2, "Cooldown B debe bajar a 2")
	assert_false(unit_b.has_moved_this_turn, "B debe poder moverse")
	
	# ── Test 5: Alter states con SEL_SELF expiran ──
	var poison = _create_poison()
	unit_a._currentAlterStates[poison] = 1
	
	await _advance_and_wait(tm)  # turno user_a — veneno expira
	
	assert_false(unit_a._currentAlterStates.has(poison), "Veneno debe haber expirado")
	
	# ── Test 6: Alter states aplican daño cada turno ──
	var poison2 = _create_poison()
	unit_b._currentAlterStates[poison2] = 3
	var hp_before_poison = unit_b.hp
	
	await _advance_and_wait(tm)  # turno user_b — veneno aplica daño
	
	assert_lt(unit_b.hp, hp_before_poison, "Veneno debe dañar a Unit B")
	assert_eq(unit_b._currentAlterStates[poison2], 2, "Veneno debe tener 2 turnos restantes")
	
	# ── Test 7: Kill desuscribe ──
	unit_b.kill()
	await wait_physics_frames(2)
	
	_assert_subscribed(tm, unit_b, false, "Unit B muerta no debe estar suscrita")
	assert_null(unit_b._tile, "Unit B muerta no debe tener tile")
	
	# ── Test 8: Cooldown llega a 0 ──
	unit_a._habilities[hab_a] = 1
	await _advance_and_wait(tm)  # turno user_b
	await _advance_and_wait(tm)  # turno user_a — cooldown baja a 0
	
	assert_eq(unit_a._habilities[hab_a], 0, "Cooldown debe llegar a 0")
	
	gut.pause_before_teardown()
	pass_test("advance_turn and tick_turn work correctly")
	gut.add_children_to = env.prev_add_target
