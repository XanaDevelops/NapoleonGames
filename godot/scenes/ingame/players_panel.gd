...
#func setup(tm: TurnManager) -> void:
	#turnManager = tm
	#var players = tm.player_deployment_data.keys()
#
	#_p1 = players[0]
	#_p2 = players[2]
	#
...
#func set_phase_battle() -> void:
	...
	#p2_num_cards.visible = false
	#
	#if not btn_end_turn.pressed.is_connected(pass_turn):
		#btn_end_turn.pressed.connect(pass_turn)
	#if not turnManager.tick_turn.is_connected(update_turn_info):
		#turnManager.tick_turn.connect(update_turn_info)
#
#func update_cards_number(player: UserRes, remaining_cards: int) -> void:
	#if player == _p1:
...
#
#func update_turn_info() -> void:
	#phase_label.text = "COMBATE - Turno %d" % turnManager.turn_number
	#set_active_player()
#
#func set_active_player() -> void:
	#var is_p1: bool = turnManager.is_player1_turn()
